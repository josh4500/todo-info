...
router.post("/update/:userid/:id", verifyKey, async (req, res) => {
  ...
  try {
    ...
    const updatedTodo = await Todo.updateOne(
      ...
      { title, isTodo, description, todoList }
    );
    res.json({
  ...
  try {
    ...
    if (userTodo == null || userTodo.length < 1)
      return res
        .status(404)
        .json({
          message: `User: ${req.params.userid} does not have any todo of id: ${req.params.id}`,
          data: { userid: req.params.userid, id: req.params.id },
        });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
